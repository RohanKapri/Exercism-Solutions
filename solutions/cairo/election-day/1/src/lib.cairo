...
pub fn increment_vote_count(
    ...
    VoteCounter {
        value: *vote_counter.value + by,
    }
}

pub fn new_election_result(
    name: ByteArray,
    votes: u32,
) -> ElectionResult {
    ElectionResult {
        name,
        vote_counter: new_vote_counter(votes),
    }
}
...
pub fn display_result(
    ...
    let mut result = election_result.name.clone();

    result += " (";
    result += u32_to_bytearray(**election_result.vote_counter.value);
    result += ")";

    result
}

pub fn decrement_votes_of_candidate(
    ref final_results: Felt252Dict<u32>,
    candidate: felt252,
) {
