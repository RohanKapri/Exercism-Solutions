...
CLASS zcl_high_scores IMPLEMENTATION.
  ...
  METHOD personalbest.
    LOOP AT scores_list ASSIGNING FIELD-SYMBOL(<curr_score>).
  ...
  METHOD personaltopthree.
    DATA(temp_scores) = scores_list.
    SORT temp_scores BY table_line DESCENDING.
    LOOP AT temp_scores ASSIGNING FIELD-SYMBOL(<curr_score>).
      IF sy-tabix > 3.
        EXIT.
      ENDIF.
      APPEND <curr_score> TO result.
    ENDLOOP.
  ENDMETHOD.
