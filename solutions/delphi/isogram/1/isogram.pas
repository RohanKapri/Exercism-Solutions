...
begin
  ...
  begin
    count := 0;
    // I count the number of appearances in the word
    for i := Low(AValue) to High(AValue) do
    begin
      if (ch = LowerCase(AValue[i])) then
        Inc(count);
      // if have more than 1 time the same letter, can exit with False
      if (count >= 2) then
      begin
        result := false;
        Break;    // not necessary continue
      end;
    end;
  end;
