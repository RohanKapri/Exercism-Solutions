...
pure string meetup(ushort year, Month month, Week week, DayOfWeek day)
{
    ...
    auto firstDay = Date(year, month, 1);

    // Calculate the offset to the desired day of the week
    int dayOffset = (day - firstDay.dayOfWeek + 7) % 7;

    // Calculate the target date
    Date targetDate;
    switch (week) {
        case Week.first:
            targetDate = firstDay + days(dayOffset);
            break;
        case Week.second:
            targetDate = firstDay + days(dayOffset + 7);
            break;
        case Week.third:
            targetDate = firstDay + days(dayOffset + 14);
            break;
        case Week.fourth:
            targetDate = firstDay + days(dayOffset + 21);
            break;
        case Week.teenth:
            // Teenth days are 13th to 19th
            targetDate = firstDay + days(dayOffset);
            while (targetDate.day < 13) {
                targetDate = targetDate + days(7);
            }
            break;
        case Week.last:
            // Find the last occurrence by iterating from the end
            int lastDayNum = 31;
            while (lastDayNum > 0) {
                try {
                    auto lastDay = Date(year, month, cast(ubyte)lastDayNum);
                    int offset = (lastDay.dayOfWeek - day + 7) % 7;
                    targetDate = lastDay - days(offset);
                    break;
                } catch (Exception) {
                    lastDayNum--;
                }
    ...
    return format("%04d-%02d-%02d", targetDate.year, targetDate.month, targetDate.day);
}
