...
pure ubyte[] encode(immutable uint[] integers)
{
    ...
    foreach (immutable value; integers)
    {
        ubyte[] bytes;
        uint v = value;  // Make a mutable copy
        
        // Special case for zero
        if (v == 0) {
            result.put(cast(ubyte)0);
            continue;
        }
        
        // Encode the value into VLQ format
        // Extract 7-bit chunks from LSB to MSB
        // All bytes get the continuation bit set initially
        do {
            bytes ~= cast(ubyte)((v & 0x7F) | 0x80);
            v >>= 7;
        } while (v > 0);
        
        // Clear the continuation bit on the first byte (LSB, which will be last after reversing)
        bytes[0] &= 0x7F;
        
        // Bytes are in reverse order, need to output them reversed
...
pure uint[] decode(immutable ubyte[] bytes)
{
    import std.array : appender;
    ...
    foreach (b; bytes)
    {
        ...
        // Always shift left by 7 and OR in the data
        value = (value << 7) | data;
        
        if ((b & 0x80) == 0)
        {
            // Last byte - output the accumulated value
            result ~= value;
            value = 0;
            shift = 0;
        }
        ...
        {
            // Continuation byte - continue accumulating
            shift += 7;
            enforce(shift < 32, "VLQ sequence is too long");
        }
    }
    
    // Check for incomplete sequence
    enforce(shift == 0, "Incomplete VLQ sequence");
