...
int[] row(immutable string numbers, size_t index)
{
    // Split by newlines to get rows
    auto rows = numbers.split("\n");
    
    // index is 1-based, convert to 0-based
    auto rowIndex = index - 1;
    
    // Get the specific row and split by spaces, then convert to integers
    return rows[rowIndex].split(" ").map!(a => a.to!int).array;
}

int[] column(immutable string numbers, size_t index)
{
    // Split by newlines to get rows
    auto rows = numbers.split("\n");
    ...
    // Extract the column from each row
    int[] result;
    foreach (r; rows)
    {
        auto cols = r.split(" ");
        result ~= cols[colIndex].to!int;
    }
