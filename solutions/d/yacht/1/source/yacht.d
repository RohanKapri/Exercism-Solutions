...
import std.algorithm : sum, equal;
...
auto countFaceRolls(immutable int[] dices)
{
    int[6] rolls = [0, 0, 0, 0, 0, 0];
    foreach (dice; dices)
        rolls[dice - 1]++;
    return rolls;
}

int valueOfNOfAKind(int[6] faceRolls, int n)
{
    foreach (index, rolls; faceRolls[].retro.enumerate)
    {
        if (rolls == n)
        {
            return cast(int)(n * (6 - index));
        }
    }
    return 0;
}

int valueOfAtLeastNOfAKind(int[6] faceRolls, int n)
{
    foreach (index, rolls; faceRolls[].retro.enumerate)
    {
        if (rolls >= n)
...
{
    auto faceRolls = countFaceRolls(dice);
    ...
    switch (category)
    {
        case Category.ones:
            return faceRolls[0];
        case Category.twos:
            return faceRolls[1] * 2;
        case Category.threes:
            return faceRolls[2] * 3;
        case Category.fours:
            return faceRolls[3] * 4;
        case Category.fives:
            return faceRolls[4] * 5;
        case Category.sixes:
            return faceRolls[5] * 6;
        case Category.fullHouse:
        {
            int threeOfAKind = valueOfNOfAKind(faceRolls, 3);
            int twoOfAKind = valueOfNOfAKind(faceRolls, 2);
            return (threeOfAKind > 0 && twoOfAKind > 0) ? threeOfAKind + twoOfAKind : 0;
        }
        case Category.fourOfAKind:
            return valueOfAtLeastNOfAKind(faceRolls, 4);
        case Category.littleStraight:
            return faceRolls[0..5].equal([1, 1, 1, 1, 1]) ? 30 : 0;
        case Category.bigStraight:
            return faceRolls[1..6].equal([1, 1, 1, 1, 1]) ? 30 : 0;
        case Category.choice:
            return dice.sum;
        case Category.yacht:
            return valueOfNOfAKind(faceRolls, 5) > 0 ? 50 : 0;
        default:
            return 0;
    }
}
