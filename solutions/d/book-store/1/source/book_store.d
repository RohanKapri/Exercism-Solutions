...
// Declare the constant array for book prices
immutable int[] bookPrices = [600, 640, 720, 760, 800];

pure int total(immutable int[] basket)
{
    auto bookCountPerType = [0, 0, 0, 0, 0, 0];
    
    foreach (book; basket) {
        ++bookCountPerType[book];
    }
    
    sort(bookCountPerType);
    
    int discountFiveBooks = bookCountPerType[1];
    int discountFourBooks = bookCountPerType[2] - bookCountPerType[1];
    int discountThreeBooks = bookCountPerType[3] - bookCountPerType[2];
    int discountTwoBooks = bookCountPerType[4] - bookCountPerType[3];
    int discountOneBook = bookCountPerType[5] - bookCountPerType[4];
    
    // Two groups of four are cheaper than a group of five plus a group of three.
    int optimalDiscountAdjustment = min(discountThreeBooks, discountFiveBooks);
    discountFiveBooks -= optimalDiscountAdjustment;
    discountThreeBooks -= optimalDiscountAdjustment;
    discountFourBooks += 2 * optimalDiscountAdjustment;
    return 5 * discountFiveBooks * bookPrices[0] + 4 * discountFourBooks * bookPrices[1] + 3 * discountThreeBooks * bookPrices[2] + 2 * discountTwoBooks * bookPrices[3] + 1 * discountOneBook * bookPrices[4];
}
