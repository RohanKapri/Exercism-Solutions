...
class Hand
  new: (@string) =>
    ...
    table.sort @cards, (a, b) -> a > b  -- sort descending by face value
    @analyze!
  ...
  straightFlush: =>
    ...
    @ranking = @isAceHighStraight! and 2 or 3
    @value = @isAceLowStraight! and 5 or @cards[1].value
    true
  ...
  isFlush: =>
    suits = {card.suit, true for card in *@cards}
    count = 0
    count += 1 for _ in pairs suits
    count == 1

  isStraight: =>
    values = List [card.value for card in *@cards]
    return true if values == {14, 5, 4, 3, 2}
    for i = 2, #values
      return false if values[i - 1] - 1 != values[i]
    true

  isAceHighStraight: => @cards[1].value == 14 and @cards[2].value == 13
  isAceLowStraight:  => @cards[1].value == 14 and @cards[2].value == 5
...
-- ---------------------------------------------------------
{
  bestHands: (handStrings) ->
    hands = [Hand hand for hand in *handStrings]
    table.sort hands
    [hand.string for hand in *hands when hand.ranking == hands[1].ranking and hand.value == hands[1].value]
}